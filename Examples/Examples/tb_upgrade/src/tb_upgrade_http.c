...
#define TIMEOUT 20000 /*SSL Timeout variable*/
...
static int32 httpReadLine(int32 sock, char* pLineBuf, int32 sizeLineBuf)
{
    ...
    while (i < sizeLineBuf)
    {
        rc = tb_ssl_read(sock, (uint8*)&c, 1, TIMEOUT); /*Receive from the SSL link*/
        if (rc <= 0)
        {
            return rc;
        }
        ...
        if (c == '\r')  // GOT CR
        {
            rc = tb_ssl_read(sock, (uint8*)&c, 1, TIMEOUT);
            if (rc <= 0)
            {
                return rc;
            }
...
static int32 httpReadBody(int32 sock, char* pBodyBuf, int32 sizeBody)
{
    ...
    return tb_ssl_read(sock, (uint8*)pBodyBuf, sizeBody, TIMEOUT);
}
...
uint8 tb_upgrade_http_Connect(TB_Upgrade_HttpContextT* pHttpContext)
{
	...
	/*After opening the socket, try to establish the SSL link*/
	pHttpContext->sockdes_ssl = tb_ssl_open(pHttpContext->sockdes, TIMEOUT);
	if (pHttpContext->sockdes_ssl < 0 )
	{
		TP_NetCloseSocket(pHttpContext->sockdes);
		pHttpContext->sockdes = -1;
		pHttpContext->sockdes_ssl = -1;
		return 0x05;
	}
                           ...
                           uint32 contentRangeStart, uint32 contentRangeEnd)
{
    ...
    while (tb_ssl_read(pHttpContext->sockdes_ssl,(uint8*)&sendSize, 1,TIMEOUT)== 1) {
    ...
    retVal = tb_ssl_write(pHttpContext->sockdes_ssl,
                             ...
                             TB_Upgrade_HttpResponseT* pResponseBuf)
{
    ...
    lenHeader = httpReadHeader(pHttpContext->sockdes_ssl,
    ...
    retVal = httpReadBody(pHttpContext->sockdes_ssl,
...
uint8 tb_upgrade_http_DisConnect(TB_Upgrade_HttpContextT* pHttpContext)
{
	...
	/*If the SSL link was established sucessfully, close it*/
	if (pHttpContext->sockdes_ssl >= 0)
	{
		tb_ssl_close(pHttpContext->sockdes_ssl);
	}

	TP_NetCloseSocket(pHttpContext->sockdes); /*close the socket*/
