...
Boolean checkHttpAddr(const char* httpAddr, uint16 httpAddrLen)
{
    ...
    TP_DbgSerialPrn("%s\x00", "Direccion HTTP:");
...
void TP_OnFotaSMSNotify(TP_SMSUpgradeInfoT* upgradeInfo_p)
{
    ...
    else if (upgradeInfo_p->type == UPGRADE_SMS)
    {
        ...
        TP_LcdPrintf((uint8*)"Actualizando...");
        ...
        if (result == 0x00)
        {
            ...
            TP_LcdPrintf((uint8*)"%s",
                         "  Instalando archivo de Actualizacion, Esto tomara aproximadamente 5 minutos. No apague el dispositivo");
        ...
        else
        {
            ...
            {
            case 0x01:
                TP_LcdPrintf((uint8*)"!Fallo Eliminacion de Bloque");
                break;
            case 0x02:
                TP_LcdPrintf((uint8*)"!Fallo Conexion al Servidor");
                break;
            case 0x03:
                TP_LcdPrintf((uint8*)"!Archivo de Actualizacion no Existe");
                break;
            case 0x04:
                TP_LcdPrintf((uint8*)"!Fallo Descarga Archivo de Actualizacion");
                break;
            case 0x05:
                TP_LcdPrintf((uint8*)"!Fallo Escritura de Archivo");
                break;
            }
                            ...
                            uint32 currValue, uint32 maxValue)
{
    ...
    if (step == UPGRADE_STEP_ERASE)
    {
        ...
        TP_LcdPrintf((uint8*)"Borrando Bloque:");
    ...
    {
        TP_ScrGotoxyEx(14, 14);
        TP_LcdPrintf((uint8*)"Conectando al Servidor:");
        TP_ScrGotoxyEx(14, 38);
        TP_LcdPrintf((uint8*)"Intentos: %d / %d", currValue, maxValue);
    }
    else if (step == UPGRADE_STEP_DOWNLOAD)
    {
        TP_ScrGotoxyEx(14, 14);
        TP_LcdPrintf((uint8*)"Descargando Archivo:");
        if (maxValue > 0)
        {
            TP_ScrGotoxyEx(14, 38);
            TP_LcdPrintf((uint8*)"%d / %d", currValue, maxValue);
        ...
        {
            TP_ScrGotoxyEx(14, 38);
            TP_LcdPrintf((uint8*)"%d / Desconocido", currValue);
        }
    }
    else if (step == UPGRADE_STEP_WRITE)
    {
        TP_ScrGotoxyEx(14, 14);
        TP_LcdPrintf((uint8*)"Escribiendo Archivo:");
        if (maxValue > 0)
        {
            TP_ScrGotoxyEx(14, 38);
            TP_LcdPrintf((uint8*)"%d / %d", currValue, maxValue);
...
void TP_AppUpgradeCheck(void)
{
    ...
    if (UpgradeFaultFlag == 1)
    {
        ...
        TP_LcdPrintf((uint8*)"Actualizacion exitosa");
    ...
    else if (UpgradeFaultFlag == 2)
    {
        ...
        TP_LcdPrintf((uint8*)"!La actualizacion fallo¡£\nPresione cualquier tacla para reinicir la actualizacion");
        ...
        TP_ScrClrRect(14, 2, 113, 61);
        TP_ScrGotoxyEx(14, 2);
        TP_LcdPrintf((uint8*)"%s", "Registrando Servicio...");
        TP_TimerSet(1, 180000);
        ...
        while (TRUE)
        {
            if (TP_TimerCheck(1) == 0)
            {
                TP_ScrClrRect(14, 2, 113, 61);
                TP_ScrGotoxyEx(14, 2);
                TP_LcdPrintf((uint8*)"%s", "!Fallo Registro de Servicio¡£\nPresiones cualquier tecla para reiniciar");
    ...
    else if (UpgradeFaultFlag == 3)
    {
        ...
        TP_ScrGotoxyEx(14, 2);
        TP_LcdPrintf((uint8*)"Fallo la Actualizacion");
