...
int32 CT_ChangePassword(const char *login, const char *password, const char *new_password){
	...
	changePasswordIso8583(&isoOut,g_userData.userCompany,g_userData.userNumber,g_userData.securityToken, userXML);
	result = sendIso8583toCrypto(&isoOut, 0);
	if(result == 0){
...
void changePasswordIso8583(iso8583 *isoOut, const char* userCompany, const char* user, const char* securityToken, const char * userDataXML){
	...
	setBit(3,isoOut->bitmap);
	strcpy((char *)isoOut->field_3,"001000");
	...
	setBit(40,isoOut->bitmap);
	strcpy((char *)isoOut->field_40,"001");

	setBit(104-64,isoOut->bitmap2);
	len = strlen(userDataXML);
	sprintf(lenStr,"%03d",len);
	strcpy((char *)isoOut->field_104_len,lenStr);
	strcpy((char *)isoOut->field_104,userDataXML);
}
